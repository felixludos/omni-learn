...
class MLP(Model, nn.Sequential):
	...
	@property
	def name(self) -> str:
		return f'MLP{len(self.hidden)}-{self.nonlin}'
	...
	def setup(self, *, device: Optional[str] = None):
		layers = self._build(self.din, self.dout, hidden=self.hidden,
							 nonlin=self.nonlin, output_nonlin=self.output_nonlin,
							 norm=self.norm, dropout=self.dropout,
							 output_norm=self.output_norm, output_dropout=self.output_dropout)
