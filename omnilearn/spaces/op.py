...
from .simple import Scalar, Vector, Pixels

from omniply.apps.gaps import gear, tool as _tool, ToolKit as _ToolKit
...
class ToolKit(_ToolKit):
	def space_of(self, gizmo: str) -> AbstractSpace:
...
class SpaceCraft(GearCraftBase):
	def __init__(self, gizmo: str, *, parse: bool = True, **kwargs):
...
class ShortcutToolCraft(ToolCraftBase):
	_Space = space
	def space(self, fn: Callable) -> space: # for use as a decorator (much like `property.setter`)
	...
	_Gear = gear
	def gear(self, fn: Callable) -> gear: # for use as a decorator (much like `property.setter`)
		if not isinstance(self._gizmo, str) and len(self._gizmo) > 1:
			raise ValueError('Cannot create a space from a tool that produces multiple gizmos')
...
class tool(SpacedToolDecorator, _tool):
	class _ToolCraft(SpacedToolCraft, ShortcutToolCraft, _tool._ToolCraft):
		pass
	class from_context(SpacedToolDecorator, _tool.from_context):
		class _ToolCraft(SpacedToolCraft, ShortcutToolCraft, _tool.from_context._ToolCraft):
			pass



class Indicator(SpacedToolDecorator):
	def __init__(self, *gizmos, space=None, parse=None, **kwargs):
		if space is None:
			space = Scalar()
		if parse is None:
			parse = False
