...
class PriorTfm:
	def __init__(self, A):
		...
		style_dim = getattr(self, 'style_dim', None)
		if style_dim is None:
			style_dim = getattr(self, 'latent_dim', None)

		if 'prior-tfm' in A and style_dim is not None:
			A.push('prior-tfm.din', style_dim, silent=True)
			A.push('prior-tfm.dout', style_dim, silent=True)

		self.prior_tfm = A.pull('prior-tfm', None)
		if self.prior_tfm is None:
			print('WARNING: no transform of the prior')
