...
class Multi_Object(object):
	...
	def __init__(self, model_path=None, neutral_pos=[0., 0, 0.], epsilon=1e-8,
				 ...
				 bg_path='/home/fleeb/Downloads/val_256/', tex_path='/home/fleeb/Downloads/dtd/images/'):
		...
		self.bg_files = util.crawl(bg_path, lambda x: '.png' in x) if mod_textures else None
		...
		self.randomize_on_reset = self.mod_shape or self.mod_size or self.mod_textures
		...
		geoms, materials, textures = mjutil.gen_object(num_prim)
		textures = textures + [mjutil.gen_skybox(self.bg_files)]
		materials = materials + mjutil.gen_label_mats()

		geoms = [mjutil.xml_format('geom', g) for g in geoms]
		textures = [mjutil.xml_format('texture', t) for t in textures]
		materials = [mjutil.xml_format('material', m) for m in materials]
		...
		self.label_maker = mjutil.Label_Maker(self.sim, body_names=['floating_obj'])
	...
	def randomize_env(self):
		...
		if self.mod_textures:
			tex = mjutil.pick_textures(self.sim, self.tex_files, self.bg_files, bg_idx=self.num_prim)
			mjutil.texture_geoms(self.sim, tex)

		if self.mod_shape or self.mod_size:
			mjutil.randomize_obj(self.sim, 'floating_obj', mod_shape=self.mod_shape, mod_size=self.mod_size)

	def simplify_env(self):
		mjutil.set_simple_shape(self.sim)#, 'floating_obj')
