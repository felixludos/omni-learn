...
class Multi_Object(object):
	...
	def __init__(self, model_path=None, epsilon=1e-8,
		...
		self.ranges = self.sim.model.jnt_range[self.sim.model.jnt_limited.astype(bool)].T
	...
	def sample_prior(self):
		loc = np.random.uniform(low=self.ranges[0], high=self.ranges[1]).reshape(self.num_obj,3)

		ori = np.random.randn(self.num_obj, 4)
		ori /= np.linalg.norm(ori, axis=-1, keepdims=True)

		pos = np.concatenate([loc, ori], -1)
	...
	def reset(self, state=None):
		...
		# else:
		assert len(state) == 13*self.num_obj
		pos = state[:7*self.num_obj]
		...
		#print('r')
		self.render(10, 10, show_label=False)
