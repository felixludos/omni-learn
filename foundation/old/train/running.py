...
def new_run_full(A, get_data=None, get_model=None, get_name=None):
	...
	if 'resume' in A:
		...
		assert not os.path.isfile(
			path), 'When resuming you should only specify the directory of the run, not the specific checkpoint'
		if 'extend' in A:
			extend = A.extend
		if 'override' in A:
			A = A.override
	...
	if 'save_dir' in A.output:
		...
		if os.environ[
			'FOUNDATION_RUN_MODE'] == 'cluster' and 'JOBDIR' in os.environ:  # cluster checkpointing for restarts
...
def run_continuous(A, records, model, trainloader, valloader=None,
	...
	print('Each training epoch has {} steps (will run for about {} epochs)'.format(len(trainloader),
	                                                                               step_limit // len(trainloader)))
	...
	while keep_going():
		...
		if not silent:
			...
			if bar is not None:
				bar.set_description(
					'Train ep={} ckpt={}{}'.format(records['epoch'] + 1, records['checkpoint'], loss_info))
		...
		if val_freq is not None and records['total_steps'] % val_freq == 0:
			...
			for i, batch in vloader:
				...
				if pbar is not None:
					loss_info = ' Loss: {:.3f} ({:.3f})'.format(val_stats['loss'].val.item(),
					                                            val_stats['loss'].smooth.item()) \
						if val_stats['loss'].count > 0 else ''
					vloader.set_description(
						'Val ep={} ckpt={}{}'.format(records['epoch'] + 1, records['checkpoint'], loss_info))
