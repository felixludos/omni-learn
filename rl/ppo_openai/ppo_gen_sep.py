...
class Agent(nn.Module):
	...
	def __init__(self):
		...
		self.norm = nn.BatchNorm1d(OBSERVATION_DIM)
	...
	def train_step(self, states, returns):
		...
		for ppo_iter in range(PPO_EPOCH):
			...
			for idx, (x, y) in enumerate(dataloader):
				...
				ypred = self(x)
				value_net_loss = F.mse_loss(ypred, y)
...
def train(agent, policy_net_opt, value_net_opt):
	...
	# 	lambda ep: (n_batch - ep) / float(n_batch),
	# 	-1)
	value_net_opt = agent.value.optim
	value_net_scheduler = agent.value.scheduler

	baseline = agent.value
	...
	for train_iter in range(n_batch):
		...
		baseline.to(device=ROLLOUT_DEVICE).eval()
		...
		baseline.to(device=TRAIN_DEVICE).train()
		...
		for ppo_iter in range(PPO_EPOCH):
			...
			for idx, (sample_obs, sample_actions, sample_adv, sample_action_logprobs) in enumerate(dataloader):
		...
		value_net_losses = []
		baseline.train_step(states, returns)
...
if __name__ == '__main__':
	...
	policy_net_opt = optim.Adam(agent.policy.parameters(), lr=LEARNING_RATE, eps=1e-5)
	value_net_opt = None #optim.Adam(agent.value.parameters(), lr=LEARNING_RATE, eps=1e-5)
