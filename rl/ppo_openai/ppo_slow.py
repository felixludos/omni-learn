...
def rollout(env, policy_net, value_net, current_observations):
	...
	for i in range(N_STEPS):
		...
		actions, observations, rewards, dones, action_means, action_logstddevs, values = \
			step_parallel_env(env, policy_net.norm(observations), policy_net, value_net)
...
def train(policy_net, value_net, policy_net_opt, value_net_opt):
	...
	for train_iter in range(n_batch):
		...
		(observations,  # Next observations
		 ...
		 batch_action_means_th,
		 batch_action_logstddevs_th,
		 batch_values_th) = rollout(env, policy_net, value_net, observations)
		...
		policy_net.norm.update(batch_obs_th)
		batch_obs_th = policy_net.norm(batch_obs_th)
