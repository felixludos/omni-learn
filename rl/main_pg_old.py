...
def main(argv=None):
	...
	if argv is not None:
		...
		parser = util.options.setup_pg_options()
	...
	agent, env = util.setup_system(args)
	
	#agent = fd.agents.Parallel_Agent(agent)
	#print('Using a parallel agent')
	...
	print('Env: {} (obs={}, act={})'.format(args.env, env.spec.obs_space.size, env.spec.action_space.size))
	print('Agent: {} (policy={}, model={}, baseline={})'.format(args.agent, args.policy, args.model, args.baseline))
	
	if args.model == 'mlp':
		print('Model: {}'.format(agent.policy.model))
	if args.baseline == 'mlp':
		print('Baseline: {}'.format(agent.baseline.model))
	...
	path_gen = Trajectory_Generator(N=args.num_iter*args.num_traj, num_workers=args.num_gen_workers, batch_size=args.num_traj,
									policy=agent.policy, args=args, seed=args.seed, def_type=args.def_type, cache=args.cache_iter)

	evaluator = Policy_Evaluator(num_workers=args.num_eval_workers, N=args.num_eval, policy=agent.policy, args=args, seed=args.seed+args.num_gen_workers)
	...
	for itr, paths in enumerate(path_gen):
		...
		# save checkpoint
		total_episodes += args.num_traj#len(paths)
		if new_best:
			saved_path = util.save_checkpoint(args, agent, logger=None, episode=total_episodes, best=best_perf, is_best=True)
			print('--- New Best Model Saved to {} ---'.format(saved_path))

		if itr % args.save_freq == 0:
			saved_path = util.save_checkpoint(args, agent, logger=statlogger, episode=total_episodes, best=best_perf, is_best=False)
	...
	if args.viz > 0:
		...
		env.visualize_policy(policy=agent.policy, N=args.viz)
