...
def main(): #
		...
		try:
			...
			while split_failed > 0:
				...
				if (counts==0).any():
					...
					if split_failed > 10:
						print('WARNING Splitting failed too many times')
						break
	...
	args.loss_type = 'nll' if j.normalized else 'cross-entropy'
	
	print('Using {} loss since model is {}normalized'.format(args.loss_type, '' if j.normalized else 'not '))
	...
	for epoch in range(args.start_epoch, args.start_epoch+args.epochs):
		...
		if 'pretend' in val_stats:
			print('*** EPOCH {}/{} pretend: train={:.4f}, val={:.4f} ***'.format(epoch+1, args.epochs+args.start_epoch, train_stats['pretend'].avg, val_stats['pretend'].avg))

		# Find best loss
		val_loss = val_stats['pretend'].avg  # all models have a flow-vel error - only from predicted/computed delta
...
def iterate(loader, model, logger, mode='test', optimizer=None, cls_criterion=None, epoch=0):
	...
	stats = util.StatsMeter('loss', 'fake', 'real', 'fake-max', 'real-min', 'pretend', 'rms-data', 'rms-gen',)
	...
	for itr, sample in enumerate(loader):
		...
		with torch.no_grad():
			...
			rms_data = x.pow(2).sum(-1).sqrt().mean()
			stats.update('rms-data', rms_data)
			
			rms_gen = audio.pow(2).sum(-1).sqrt().mean()
			...
			stats.update('loss', fk)
			stats.update('fake', fk)
